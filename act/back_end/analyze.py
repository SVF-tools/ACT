...
def analyze(net: Net, entry_id: int, entry_fact: Fact, eps: float=1e-9) -> Tuple[Dict[int, Fact], Dict[int, Fact], ConSet]:
    ...
    while WL:
        ...
        # Preserve incoming constraints (e.g., INPUT_SPEC) by merging them
        # with constraints generated by the current layer's transfer function.
        for con in before[lid].cons:
            out_fact.cons.replace(con)
