...
def tf_relu(L: Layer, Bin: Bounds) -> Fact:
    ...
    off = u <= 0
    amb = ~(on | off)
    lb = torch.where(off, 0.0, torch.where(on, l, 0.0))
    ub = torch.where(off, 0.0, torch.where(on, u, u))
    B = Bounds(lb, ub)
    C = ConSet()
    if l.dim() <= 1:
        if torch.any(amb):
            ...
            s = torch.where(
                finite, ua / torch.clamp(gap, min=1e-12), torch.ones_like(gap)
            )
...
def tf_lrelu(L: Layer, Bin: Bounds) -> Fact:
    ...
    if l.dim() <= 1:
        if torch.any(amb):
            ...
            s = torch.where(
                finite,
                (ua - a * la) / torch.clamp(gap, min=1e-12),
                torch.full_like(gap, max(a, 1.0)),
            )
...
def tf_sub(L: Layer, Bx: Bounds, By: Bounds) -> Fact:
    ...
    assert B.lb.numel() == len(L.out_vars), (
        f"sub out_vars length {len(L.out_vars)} != output elements {B.lb.numel()}"
    )
...
def tf_mul(L: Layer, Bx: Bounds, By: Bounds) -> Fact:
    cand = torch.stack(
        [Bx.lb * By.lb, Bx.lb * By.ub, Bx.ub * By.lb, Bx.ub * By.ub], dim=0
    )
...
def tf_div(L: Layer, Bx: Bounds, By: Bounds) -> Fact:
    ...
    assert not torch.any(
        torch.isclose(ly, torch.zeros_like(ly))
        & torch.isclose(uy, torch.zeros_like(uy))
    ), "div denominator interval collapses to zero"
    cand = torch.stack(
        [
            Bx.lb / ly,
            Bx.lb / uy,
    ...
    B = Bounds(lb, ub)
    assert B.lb.numel() == len(L.out_vars), (
        f"div out_vars length {len(L.out_vars)} != output elements {B.lb.numel()}"
    )
    assert torch.all(torch.isfinite(B.lb) & torch.isfinite(B.ub)), (
        "div produced non-finite bounds"
    )
    ...
    C.replace(
        Con(
            "INEQ",
            tuple(L.out_vars + L.params["x_vars"] + L.params["y_vars"]),
            {"tag": f"div:{L.id}", "safe": not torch.any(crosses_zero).item()},
        )
    )
    C.add_box(L.id, L.out_vars, B)
    return Fact(B, C)
...
def tf_matmul(L: Layer, Bx: Bounds, By: Bounds) -> Fact:
    ...
    # Vectorized: broadcast [m,k,1] * [1,k,n] -> [4,m,k,n]
    products = torch.stack(
        [
            X_lb.unsqueeze(2) * Y_lb.unsqueeze(0),
            X_lb.unsqueeze(2) * Y_ub.unsqueeze(0),
            X_ub.unsqueeze(2) * Y_lb.unsqueeze(0),
            X_ub.unsqueeze(2) * Y_ub.unsqueeze(0),
        ],
        dim=0,
    )
    out_lb = products.min(dim=0).values.sum(dim=1).reshape(-1)
    out_ub = products.max(dim=0).values.sum(dim=1).reshape(-1)
...
def tf_sigmoid(L: Layer, Bin: Bounds) -> Fact:
    ...
    B = Bounds(f(Bin.lb), f(Bin.ub))
    C = ConSet()
    C.replace(
        Con(
            "INEQ",
            tuple(L.out_vars + L.in_vars),
            {"tag": f"sigmoid:{L.id}", "segs": pwl_meta(Bin.lb, Bin.ub, 2)},
        )
    )
    C.add_box(L.id, L.out_vars, B)
    return Fact(B, C)
...
def tf_tanh(L: Layer, Bin: Bounds) -> Fact:
    ...
    C.replace(
        Con(
            "INEQ",
            tuple(L.out_vars + L.in_vars),
            {"tag": f"tanh:{L.id}", "segs": pwl_meta(Bin.lb, Bin.ub, 2)},
        )
    )
    C.add_box(L.id, L.out_vars, B)
    return Fact(B, C)
...
def tf_softplus(L: Layer, Bin: Bounds) -> Fact:
    ...
    C.replace(
        Con(
            "INEQ",
            tuple(L.out_vars + L.in_vars),
            {"tag": f"softplus:{L.id}", "segs": pwl_meta(Bin.lb, Bin.ub, 2)},
        )
    )
    C.add_box(L.id, L.out_vars, B)
    return Fact(B, C)
...
def tf_silu(L: Layer, Bin: Bounds) -> Fact:
    ...
    C.replace(
        Con(
            "INEQ",
            tuple(L.out_vars + L.in_vars),
            {"tag": f"silu:{L.id}", "s_lb": s_lb, "s_ub": s_ub},
        )
    )
    C.add_box(L.id, L.out_vars, B)
    return Fact(B, C)
...
def tf_max(L: Layer, By_list: List[Bounds]) -> Fact:
    lb = torch.maximum.reduce([b.lb for b in By_list])
    ub = torch.maximum.reduce([b.ub for b in By_list])
    B = Bounds(lb, ub)
    ...
    C.replace(
        Con(
            "INEQ",
            tuple(L.out_vars + all_y),
            {"tag": f"max:{L.id}", "k": len(By_list), "mode": "convex"},
        )
    )
    C.add_box(L.id, L.out_vars, B)
    return Fact(B, C)
...
def tf_min(L: Layer, By_list: List[Bounds]) -> Fact:
    lb = torch.minimum.reduce([b.lb for b in By_list])
    ub = torch.minimum.reduce([b.ub for b in By_list])
    B = Bounds(lb, ub)
    all_y = sum((L.params["y_vars_list"][i] for i in range(len(By_list))), [])
    C = ConSet()
    C.replace(
        Con(
            "INEQ",
            tuple(L.out_vars + all_y),
            {"tag": f"min:{L.id}", "k": len(By_list), "mode": "convex"},
        )
    )
    C.add_box(L.id, L.out_vars, B)
    return Fact(B, C)
...
def tf_square(L: Layer, Bin: Bounds) -> Fact:
    ...
    C.replace(
        Con(
            "INEQ",
            tuple(L.out_vars + L.in_vars),
            {"tag": f"square:{L.id}", "segs": pwl_meta(l, u, 2)},
        )
    )
    C.add_box(L.id, L.out_vars, B)
    return Fact(B, C)
...
def tf_power(L: Layer, Bin: Bounds) -> Fact:
    ...
    C.replace(
        Con(
            "INEQ",
            tuple(L.out_vars + L.in_vars),
            {"tag": f"power:{L.id}", "p": p, "segs": pwl_meta(Bin.lb, Bin.ub, 2)},
        )
    )
    C.add_box(L.id, L.out_vars, B)
    return Fact(B, C)
...
def tf_hardtanh(L: Layer, Bin: Bounds) -> Fact:
    ...
    C.replace(
        Con(
            "INEQ",
            tuple(L.out_vars + L.in_vars),
            {"tag": f"hardtanh:{L.id}", "min_val": min_val, "max_val": max_val},
        )
    )
    C.add_box(L.id, L.out_vars, B)
    return Fact(B, C)
...
def tf_hardsigmoid(L: Layer, Bin: Bounds) -> Fact:
    ...
    C.replace(
        Con(
            "INEQ",
            tuple(L.out_vars + L.in_vars),
            {"tag": f"hardsigmoid:{L.id}", "alpha": alpha, "beta": beta},
        )
    )
    C.add_box(L.id, L.out_vars, B)
    return Fact(B, C)
...
def tf_reshape(L: Layer, Bin: Bounds) -> Fact:
    ...
    C.replace(
        Con(
            "EQ",
            tuple(L.out_vars + L.in_vars),
            {"tag": f"reshape:{L.id}", "target_shape": L.params.get("target_shape")},
        )
    )
    C.add_box(L.id, L.out_vars, B)
    return Fact(B, C)
...
def tf_transpose(L: Layer, Bin: Bounds) -> Fact:
    ...
    C.replace(
        Con(
            "EQ",
            tuple(L.out_vars + L.in_vars),
            {"tag": f"transpose:{L.id}", "perm": L.params.get("perm")},
        )
    )
    C.add_box(L.id, L.out_vars, B)
    return Fact(B, C)
...
def tf_squeeze(L: Layer, Bin: Bounds) -> Fact:
    ...
    C.replace(
        Con(
            "EQ",
            tuple(L.out_vars + L.in_vars),
            {"tag": f"squeeze:{L.id}", "dims": L.params.get("dims")},
        )
    )
    C.add_box(L.id, L.out_vars, B)
    return Fact(B, C)
...
def tf_unsqueeze(L: Layer, Bin: Bounds) -> Fact:
    ...
    C.replace(
        Con(
            "EQ",
            tuple(L.out_vars + L.in_vars),
            {"tag": f"unsqueeze:{L.id}", "dims": L.params.get("dims")},
        )
    )
    C.add_box(L.id, L.out_vars, B)
    return Fact(B, C)
...
def tf_tile(L: Layer, Bin: Bounds) -> Fact:
    ...
    C.replace(
        Con(
            "EQ",
            tuple(L.out_vars + L.in_vars),
            {"tag": f"tile:{L.id}", "repeats": repeats},
        )
    )
    C.add_box(L.id, L.out_vars, B)
    return Fact(B, C)
...
def tf_expand(L: Layer, Bin: Bounds) -> Fact:
    ...
    C.replace(
        Con(
            "EQ",
            tuple(L.out_vars + L.in_vars),
            {"tag": f"expand:{L.id}", "shape": L.params.get("shape")},
        )
    )
    C.add_box(L.id, L.out_vars, B)
    return Fact(B, C)
...
def tf_slice(L: Layer, Bin: Bounds) -> Fact:
    ...
    assert out_lb.numel() == len(L.out_vars), (
        f"slice out_vars length {len(L.out_vars)} != output elements {out_lb.numel()}"
    )
...
def tf_index_select(L: Layer, Bin: Bounds) -> Fact:
    ...
    assert 0 <= dim < len(inp_shape), (
        f"index_select dim {dim} out of range for input shape {inp_shape}"
    )
    x_lb = Bin.lb.view(*inp_shape)
    x_ub = Bin.ub.view(*inp_shape)
    ...
    assert out_lb.numel() == len(L.out_vars), (
        f"index_select out_vars length {len(L.out_vars)} != output elements {out_lb.numel()}"
    )
...
def tf_reorder(L, ctx):
    ...
    assert order.numel() == lx.shape[dim], (
        f"reorder order length {order.numel()} != dim size {lx.shape[dim]}"
    )
...
def tf_stack(L, ctx):
    ...
    assert stacked_lb.shape == stacked_ub.shape, (
        "stack produced mismatched bound shapes"
    )
    assert torch.all(stacked_lb <= stacked_ub), (
        "stack produced invalid bounds (lb > ub)"
    )
