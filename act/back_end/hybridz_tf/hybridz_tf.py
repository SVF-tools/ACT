...
class HybridzTF(TransferFunction):
    ...
    _LAYER_REGISTRY = {
        ...
        # Activations
        "CLIP": lambda L, bounds, tf: hybridz_tf_clip(L, bounds),
        "SOFTPLUS": lambda L, bounds, tf: hybridz_tf_softplus(L, bounds),
        "SILU": lambda L, bounds, tf: hybridz_tf_silu(L, bounds),
        "RELU6": lambda L, bounds, tf: hybridz_tf_relu6(L, bounds),
        "HARDTANH": lambda L, bounds, tf: hybridz_tf_hardtanh(L, bounds),
        "HARDSIGMOID": lambda L, bounds, tf: hybridz_tf_hardsigmoid(L, bounds),
        "HARDSWISH": lambda L, bounds, tf: hybridz_tf_hardswish(L, bounds),
        "MISH": lambda L, bounds, tf: hybridz_tf_mish(L, bounds),
        "SOFTSIGN": lambda L, bounds, tf: hybridz_tf_softsign(L, bounds),
        # Element-wise / multi-input
        "BN": lambda L, bounds, tf: hybridz_tf_bn(L, bounds),
        "CONCAT": lambda L, bounds, tf: hybridz_tf_concat(
            L, tf._net.get_all_predecessor_bounds(L.id, tf._after, tf._before)
        ),
        "MAX": lambda L, bounds, tf: hybridz_tf_max(
            L, tf._net.get_all_predecessor_bounds(L.id, tf._after, tf._before)
        ),
        "MIN": lambda L, bounds, tf: hybridz_tf_min(
            L, tf._net.get_all_predecessor_bounds(L.id, tf._after, tf._before)
        ),
        ...
        "TILE": lambda L, bounds, tf: hybridz_tf_tile(L, bounds),
        "EXPAND": lambda L, bounds, tf: hybridz_tf_expand(L, bounds),
        # CNN variants
        "CONV1D": lambda L, bounds, tf: hybridz_tf_conv1d(L, bounds),
        "CONV3D": lambda L, bounds, tf: hybridz_tf_conv3d(L, bounds),
        "CONVTRANSPOSE2D": lambda L, bounds, tf: hybridz_tf_convtranspose2d(L, bounds),
    }
    ...
    def _hz_transform(self, L: Layer, hz_in: HZono) -> Optional[HZono]:
        ...
        if k == "BN":
            ...
            return hz_add_const(hz, c.view(-1, 1) if c.ndim == 1 else c)
        if k in ("FLATTEN", "RESHAPE", "TRANSPOSE", "SQUEEZE", "UNSQUEEZE", "EXPAND"):
            return hz_in
        if k == "CONCAT":
            preds = self._net.preds.get(L.id, [])
            parts = [self._hz_cache.get(pid) for pid in preds]
