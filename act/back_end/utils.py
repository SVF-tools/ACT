...
def changed_or_maskdiff(
    L, B: Bounds, masks: Optional[Dict[str, torch.Tensor]], eps=1e-9
) -> bool:
    plb = L.cache.get("prev_lb")
    pub = L.cache.get("prev_ub")
    if plb is None or pub is None:
        return True
    if torch.any(torch.abs(plb - B.lb) > eps) or torch.any(torch.abs(pub - B.ub) > eps):
        return True
    ...
    for k in masks.keys():
        if (
            (k not in prev)
            or (masks[k].shape != prev[k].shape)
            or torch.any(masks[k] != prev[k])
        ):
            return True
    return False
...
def update_cache(L, B: Bounds, masks: Optional[Dict[str, torch.Tensor]]):
    L.cache["prev_lb"] = B.lb.clone()
    L.cache["prev_ub"] = B.ub.clone()
    L.cache["masks"] = (
        None if masks is None else {k: v.clone() for k, v in masks.items()}
    )
...
def bound_var_interval(l: torch.Tensor, u: torch.Tensor) -> Tuple[float, float]:
    r = 0.5 * (u - l)
    v_hi = float(torch.mean((2 * r) ** 2))
    return (0.0, v_hi)


def scale_interval(cx_lo, cx_hi, inv_lo, inv_hi):
    cand = torch.stack(
        [cx_lo * inv_lo, cx_lo * inv_hi, cx_hi * inv_lo, cx_hi * inv_hi], dim=0
    )
        ...
        for i, var_id in enumerate(layer.out_vars):
            if var_id in var_ids_used:
                ...
                var_bounds[var_id] = Bounds(
                    lb=fact.bounds.lb[
                        i : i + 1
                    ],  # Keep as 1D tensor with single element
...
def parse_input_shape(input_shape) -> Tuple[int, int, int]:
    """Extract (C, H, W) from input_shape of format (N,C,H,W) or (C,H,W)."""
