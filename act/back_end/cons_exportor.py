...
def _add_tanh_convex_segment(
    solver: Solver, yi: int, zi: int, lo: float, hi: float
) -> None:
    if hi - lo <= TANH_EPS:
        return
    f_lo = _tanh_value(lo)
...
def _add_tanh_constraints_for_var(
    solver: Solver, yi: int, zi: int, lo: float, hi: float
) -> None:
    ...
    if max_abs <= TANH_IDENTITY_WINDOW:
        delta = max(
            abs(_tanh_value(lo) - lo), abs(_tanh_value(hi) - hi), TANH_IDENTITY_TOL
        )
        solver.add_lin_le([zi, yi], [1.0, -1.0], float(delta))
        solver.add_lin_ge([zi, yi], [1.0, -1.0], float(-delta))
        return
...
def export_to_solver(
    ...
    if hasattr(solver, "n") and solver.n == 0:
        ...
        print(
            f"🔧 export_to_solver: Solver already initialized (current vars: {getattr(solver, 'n', 'unknown')})"
        )
    ...
    for con in templates:
        ...
        if tag.startswith("dense:"):
            ...
            for k, i in enumerate(to_numpy(meta["idx_amb"]).astype(int)):
                ...
                solver.add_lin_le(
                    [z[i], y[i]], [1.0, -float(slope[k])], float(shift[k])
                )
            ...
            y = list(con.var_ids[2 * n :])
            lx, ux, ly, uy = map(
                to_numpy, (meta["lx"], meta["ux"], meta["ly"], meta["uy"])
            )
            for i in range(n):
                solver.add_lin_ge(
                    [z[i], y[i], x[i]],
        ...
        elif tag.startswith(("conv2d:", "conv1d:", "conv3d:", "convtranspose2d:")):
            W = to_numpy(con.meta["W"])
            b = to_numpy(con.meta["b"])
            n_out, n_in = W.shape
            y = list(con.var_ids[:n_out])
            ...
            for i, yi in enumerate(y):
                solver.add_lin_eq(
                    [yi] + x,
                    [1.0] + [-float(W[i, j]) for j in range(n_in)],
                    float(b[i]),
                )

        elif tag.startswith(("maxpool2d:", "avgpool2d:", "flatten:", "reshape:")):
            continue

        elif tag.startswith(("lstm:", "gru:", "rnn:", "embedding:")):
            continue
            ...
            logging.getLogger(__name__).debug("unhandled tag %r", tag)
