#===- act/back_end/solver/hz_bounds.py - Hybrid Zonotope Solver ----------====#
...
#
# Purpose:
#   Self-contained Hybrid Zonotope (HZ) solver for computing tight bounds.
#   Provides:
...
def hz_apply_leaky_relu(hz: HZono, alpha_arg: float) -> HZono:
    ...
    eq_Ac[r0, col_g1] = 1.0;  eq_Ac[r0, col_s1p] = 1.0
    eq_Ab[r0, col_z] = 0.5;   eq_b[r0, 0] = 0.5

    eq_Ac[r1, col_g1] = -1.0; eq_Ac[r1, col_s1m] = 1.0
    eq_Ab[r1, col_z] = 0.5;   eq_b[r1, 0] = 0.5

    eq_Ac[r2, col_g2] = 1.0;  eq_Ac[r2, col_s2p] = 1.0
    eq_Ab[r2, col_z] = -0.5;  eq_b[r2, 0] = 0.5

    eq_Ac[r3, col_g2] = -1.0; eq_Ac[r3, col_s2m] = 1.0
    eq_Ab[r3, col_z] = -0.5;  eq_b[r3, 0] = 0.5
...
def hz_apply_piecewise(hz: HZono, func, dfunc, K: int = 2) -> HZono:
    ...
    for k_idx in range(K):
        ...
        p2 = a + b - p1
        g1x_tang = (p1 - a) / 2.0;          g1y_tang = lb_slope * (p1 - a) / 2.0
        g2x_tang = (p2 - a) / 2.0;          g2y_tang = la * (p2 - a) / 2.0
        ...
        # Soundness check
        dx = pts - cx.unsqueeze(0);  dy = f_pts - cy.unsqueeze(0)
        det = g1y * g2x - g1x * g2y
        ...
        g1x *= scale_factor; g1y *= scale_factor; g2x *= scale_factor; g2y *= scale_factor

        centers_x_k.append(cx); centers_y_k.append(cy)
        g1_x_k.append(g1x); g1_y_k.append(g1y); g2_x_k.append(g2x); g2_y_k.append(g2y)
    ...
    ng_total = ng + n_real + n_slack;  nb_total = nb + K * m

    n_box = 4 * K * m;  n_eq_total = n_box + m + m
    ...
    for k_idx in range(K):
        for j in range(m):
            g1_col = ng + k_idx * m + j;  g2_col = ng + K * m + k_idx * m + j
            z_col = nb + k_idx * m + j;   s_base = ng + n_real + (k_idx * m + j) * 4
            r = 4 * (k_idx * m + j)
            eq_Ac[r, g1_col] = 1.0;     eq_Ac[r, s_base] = 1.0
            eq_Ab[r, z_col] = -0.5;     eq_b[r, 0] = 0.5
            eq_Ac[r+1, g1_col] = -1.0;  eq_Ac[r+1, s_base+1] = 1.0
            eq_Ab[r+1, z_col] = -0.5;   eq_b[r+1, 0] = 0.5
            eq_Ac[r+2, g2_col] = 1.0;   eq_Ac[r+2, s_base+2] = 1.0
            eq_Ab[r+2, z_col] = -0.5;   eq_b[r+2, 0] = 0.5
            eq_Ac[r+3, g2_col] = -1.0;  eq_Ac[r+3, s_base+3] = 1.0
            eq_Ab[r+3, z_col] = -0.5;   eq_b[r+3, 0] = 0.5

    for j in range(m):
        idx_i = int(wide_idx[j].item());  r = n_box + j;  rhs_val = 0.0
        for k_idx in range(K):
            g1_col = ng + k_idx * m + j;  g2_col = ng + K * m + k_idx * m + j
            z_col = nb + k_idx * m + j;   rhs_val += centers_x_k[k_idx][j].item() / 2.0
            eq_Ac[r, g1_col] = -g1_x_k[k_idx][j]
            eq_Ac[r, g2_col] = -g2_x_k[k_idx][j]
            eq_Ab[r, z_col] = centers_x_k[k_idx][j] / 2.0
        eq_Ac[r, :ng] = hz.Gc[idx_i];  eq_Ab[r, :nb] = hz.Gb[idx_i]
    ...
    old_Ac_ext = torch.cat([hz.Ac, torch.zeros((nc, n_real + n_slack), dtype=dtype, device=device)], dim=1)
    old_Ab_ext = torch.cat([hz.Ab, torch.zeros((nc, K * m), dtype=dtype, device=device)], dim=1)
...
def hz_apply_sigmoid(hz: HZono, K: int = 2) -> HZono:
    ...
    return hz_apply_piecewise(hz, torch.sigmoid,
...
def hz_apply_tanh(hz: HZono, K: int = 2) -> HZono:
    ...
    return hz_apply_piecewise(hz, torch.tanh,
...
def hz_conv2d(hz: HZono, weight, bias, stride, padding, dilation, groups, input_shape) -> HZono:
...
def hz_reduce(hz: HZono, max_order: float = 10.0) -> HZono:
    ...
    if nb > max_nb:
        ...
        ng = hz.Gc.shape[1];  nb = hz.Gb.shape[1]

    # Step 2: Reduce continuous generators
    if ng > max_ng:
        col_norms = hz.Gc.abs().sum(dim=0)
        _, sorted_idx = col_norms.sort(descending=True)
        keep_idx = sorted_idx[:max_ng - n];  drop_idx = sorted_idx[max_ng - n:]
        Gc_keep = hz.Gc[:, keep_idx]
        new_Gc = torch.cat([Gc_keep, torch.diag(hz.Gc[:, drop_idx].abs().sum(dim=1))], dim=1)

        if nc > 0:
            drop_set = set(drop_idx.tolist())
            keep_rows = [r for r in range(nc)
                         if not any(abs(hz.Ac[r, c].item()) > 1e-15 for c in drop_set)]
            if keep_rows:
                krt = torch.tensor(keep_rows, dtype=torch.long, device=device)
                new_Ac = torch.cat([hz.Ac[krt][:, keep_idx],
                                    torch.zeros((len(keep_rows), n), dtype=dtype, device=device)], dim=1)
                new_Ab = hz.Ab[krt];  new_b = hz.b[krt]
            else:
...
def _hz_compute_bounds_gurobi(hz: HZono) -> Bounds:
    ...
    n, m_c = Gc_np.shape;  _, m_b = Gb_np.shape;  p = b_np.shape[0]

    model = gp.Model("hz_bounds");  model.Params.OutputFlag = 0
    ...
    if p > 0:
        lhs = 0
        if xi_c is not None: lhs = lhs + Ac_np @ xi_c
        if xi_b is not None: lhs = lhs + Ab_np @ xi_b
    ...
    for i in range(n):
        ...
        model.setObjective(expr, GRB.MAXIMIZE);  model.optimize()
        if model.status != GRB.OPTIMAL:
            raise RuntimeError(f"Gurobi MAX infeasible at dim {i}, status={model.status}")
        UB[i] = model.ObjVal
        model.setObjective(expr, GRB.MINIMIZE);  model.optimize()
        if model.status != GRB.OPTIMAL:
