...
from .utils import (
    ...
    as_block_param,
    ensure_batch1,
    infer_conv2d_output_hw,
    infer_pool2d_output_hw,
    prod,
)
...
def append_conv2d(
    ...
    *,
    ...
    out_H, out_W = infer_conv2d_output_hw(H, W, kernel=kernel, stride=stride, padding=padding)
...
def append_pool2d(
    ...
    *,
    ...
    input_shape = [1, int(in_ch), int(H), int(W)]
    out_H, out_W = infer_pool2d_output_hw(H, W, kernel=kernel, stride=stride, padding=padding)
    if out_H <= 0 or out_W <= 0:
...
def build_mlp_layers(layers: List[Dict[str, Any]], *, cfg: Dict[str, Any]) -> None:
    ...
    shape = ensure_batch1(tuple(cfg["input_shape"]))
    in_features = int(shape[1]) if len(shape) == 2 else prod(shape[1:])

    if len(shape) > 2:
        layers.append({"kind": "FLATTEN", "params": {}, "meta": {"start_dim": 1}})

    act_kind = activation_kind(cfg["activation"])
    ...
    *,
    ...
    if cfg["variant"] == "plain":
        ...
        for i, out_ch in enumerate(cfg["conv_channels"]):
            k = as_block_param(cfg["kernel_sizes"], i, n_blocks, "kernel_sizes")
            s = as_block_param(cfg["strides"], i, n_blocks, "strides")
            p = as_block_param(cfg["paddings"], i, n_blocks, "paddings")
